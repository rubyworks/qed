...
This release makes a minor change, but it is one that can a significant
effect on end-users, so it is IMPORTANT TO READ THIS RELEASE NOTICE!
QED no longer depends on AE for its assertion framework. Instead any
compliant framework can be used. Certainly most users will be happy
to continue using AE, in which case simply add `require 'ae'` to an
applique (e.g. in 'qed/applique/ae.rb') or add `-r ae` the qed command
line invocation to ensure demos continue to run as usual.

Changes:

* No longer depend on AE only for assertions framework.
