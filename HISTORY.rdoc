...
In most respects the changes will not effect older QED documents, but it
does effect block arity for applique rules in one respect. Whereas prior
non-code blocks put the example text at the end of the arguments, now
the text is added to the beginning. In most cases this will not be an
issue b/c only the example text was used anyway, but in some cases
applique may need adjustment. In addition to this, a new notation for
captures has been added. With it, captures can now be written as `/(\S+)/`
and non-capture expressions as `/?:\S+/`. The olkd notation still works.
This release also entails a few other changes. See the change list below
for details.
...
* Example text argument proceeds match arguments.
