...
IT IS IMPORTANT TO READ THIS RELEASE NOTICE! This release makes a minor
change, but it is one that can a significant effect on end-users. That
is, QED no longer depends on AE for its sole assertions framework. Instead
any compliant framework can be used. Certainly most users will be happy
to continue using AE, in which case simply add `require 'ae'` to the
applique (e.g. in 'spec/applique/ae.rb') or add `-r ae` to the qed command
line invocation to ensure demos continue running as before.

Changes:

* No longer depend only on AE for assertions framework.
