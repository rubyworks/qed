...
With this version QED has reached a significant design milestone.
The underlying code base has a significantly better API that
should stand the test of time. In most repsects the changes will
not effect older QED documents, but it does effect block arity for
applique rules. Whereas previously the arity matched the number
of Regexp captures, now all the captures are passed to the block
as a single argument. This change can be delt with in one of two
ways, either accept the single argument and splat it to varaibles
in the subsequenct code block, or, if using Ruby 1.9+, use a
parenthetical argument group, e.g.

   When '(((\S+))) or (((\S+)))' do |(a,b)|
     ...
   end

In addition a new notation for captures has been added. With it 
captures can now be written as /(\S+)/ and non-capture regular
expressions simplified to /?:\S+/.

Changes:

* Deprecate use of '...' as plain text example indicator.
* Support for lowercase advice methods.
* Use confection gem for configuration.
* Must use -f for most reporters.
* Overhaul evaluator using step subclasses.
* Use single match argument for block parameters.
* More robust handling of rule arguments.
* Show full backtrace in verbatim reporter.
* Underlying observer API redesigned.
