...
Thie release simply fixes dependencies. It should require the RC gem instead
of the Courtier gem (which is the old name for RC gem).

Changes:

* Fix requirements to use RC.


== 2.9.0 | 2012-05-19

IMPORTANT! The `.qed` configuration file is no longer used for configuration.
Either use a unified `.ruby/.rubyrc` file or require a special config file
