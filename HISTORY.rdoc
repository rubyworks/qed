...
Configuration is now handled by the Courtier library. No other
form of configuration is supported except that of loading a file
directly via `-r ./path/to/config-file.rb`. This keeps things as
simple as possible but still fully capable. Because the old `.qed`
configuration file is no longer supported, this release gets
a more significant version bump.

Changes:

* Utilize Courtier configuration management library.
* Improve error message of hash-arrow assertion notation.
