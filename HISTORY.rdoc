...
Along with this about face, profiles are now specified via the `-p`/`--profile`
command line option instead of specialized options. In short, this release
reverts the previous release with a few addtional adjustments.

Changes:

* Use confection library for configuration.
* Use `-p`/`--profile` to select configuration profiles.
* Use `.qed-override` file to circumvent confection.
* Deprecated use of `.map` file.
