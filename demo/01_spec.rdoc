...
for Inteface-driven Development. The documnts are divided up into
clauses separated by blank lines. Clauses that are flush to the 
left margin are always explination or comment clauses. Indented
...
i.e. that is should be excecuted but not tested. So far this
such a festure has proven to be a YAGNI. Yet we may add such a
...
Only use *before* and *after* clasues when necessary --specifications
...
The arity of the table block dermines the number of columns each row
